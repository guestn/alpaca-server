...
const reducer = (state = initialState, action: Action) => {
  switch (action.type) {
    ...
    case at.SAVE_TRADE_UPDATE_SUCCEEDED:
      return {
        ...state,
        tradeUpdates: [...state.tradeUpdates, action.data],
      };
    ...
    case at.POST_ALERT_SUCCEEDED:
      const existingAlertIdx = state.alerts.findIndex((alert) => alert.id === action.data.id);
      let updatedAlerts;
      if (existingAlertIdx > -1) {
        updatedAlerts = [...state.alerts];
        updatedAlerts[existingAlertIdx] = action.data;
      } else {
        updatedAlerts = [...state.alerts, action.data];
      }
      return {
        ...state,
    ...
    case at.CREATE_ORDER_SUCCEEDED:
      return {
        ...state,
        newOrders: [...state.newOrders, action.data],
      };
